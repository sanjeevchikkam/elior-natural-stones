...
export default function CategoriesSection() {
  return (
    <section className="py-20 bg-[#0B0B0B] border-t border-white/5 relative z-10" id="categories-section">
      ...
      <div className="absolute top-1/4 left-1/2 -translate-x-1/2 w-[500px] h-[500px] bg-[#D4AF37]/3 blur-[150px] rounded-full pointer-events-none" />
      ...
      <div className="max-w-7xl mx-auto px-6 md:px-12 relative z-10">
        <div className="text-center max-w-3xl mx-auto mb-16" id="categories-header-wrapper">
          ...
          >
            <span className="text-[10px] md:text-xs text-[#D4AF37] font-mono tracking-[0.4em] uppercase" id="categories-pre-title">
              Our Collections
            </span>
          ...
          {CATEGORIES.map((cat, index) => {
            ...
            return (
              <motion.div
                ...
                className="group relative h-[360px] rounded-sm overflow-hidden border border-white/10 hover:border-[#D4AF37]/50 bg-[#121212] flex flex-col justify-end p-6 transition-all duration-300 shadow-xl"
              ...
              >
                ...
                <div className="relative z-20 flex flex-col gap-3" id={`category-card-content-${cat.id}`}>
                  <span className="text-[10px] text-[#D4AF37] font-mono tracking-widest uppercase" id={`category-card-number-${cat.id}`}>
                    Collection 0{index + 1}
                  </span>
                  
                  <h3 className="text-xl font-serif font-bold text-white tracking-wide group-hover:text-[#D4AF37] transition-colors duration-300" id={`category-card-title-${cat.id}`}>
                    {cat.name}
                  </h3>
                  ...
                  <Link
                    ...
                    className="inline-flex items-center gap-2 text-[10px] text-[#D4AF37] font-mono tracking-[0.2em] uppercase mt-2 focus:outline-none"
                ...
                <div className="absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r from-transparent via-[#D4AF37] to-transparent scale-x-0 group-hover:scale-x-100 transition-transform duration-500 origin-center z-30" />
              </motion.div>
            );
          })}
        </div>
